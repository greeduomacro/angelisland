...
namespace Server.Scripts.Commands
{
	public class Batch : BaseCommand
	{
		...
		public bool Run(Mobile from)
		{
			...
			string[] args = Server.Commands.Split(m_Condition);
		...
		{
			Server.Commands.Register("Batch", AccessLevel.Counselor, new CommandEventHandler(Batch_OnCommand));
		}
	...
	public class BatchCommand
	{
		...
		public void GetDetails(out string command, out string argString, out string[] args)
		{
			...
			if (indexOf >= 0)
			{
				...
				args = Server.Commands.Split(argString);
			}
			else
			{
				argString = "";
