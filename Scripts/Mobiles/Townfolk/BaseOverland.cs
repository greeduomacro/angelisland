...
namespace Server.Mobiles
{
	public class BaseOverland : BaseCreature
	{
		...
		{
			Server.Commands.Register("WipeOverlandMobs", AccessLevel.Administrator, new CommandEventHandler(WipeOverlandMobs_OnCommand));
		}
		...
		private class MsgTimer : Timer
		{
			...
			public MsgTimer( BaseOverland m, DateTime time ) : base( time - DateTime.Now )
			{
				m_mobile = m;
		...
		[Constructable]
		public BaseOverland() : base( AIType.AI_Melee, FightMode.Aggressor, DefaultRangePerception, 1, 0.2, 0.4 )
		{
			InitBody();
			InitOutfit();
		...
		public BaseOverland( Serial serial ) : base( serial )
		{
		}
