...
namespace Server.Gumps
{
	public class AddDoorGump : Gump
	{
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			else
			{
				if ( button >= 0 && button < 8 )
				{
					...
					Commands.Handle( from, String.Format( "{0}Add {1} {2}", Commands.CommandPrefix, m_Types[m_Type].m_Type.Name, (DoorFacing)button ) );
				}
				else if ( button == 8 )
				{
					from.SendGump( new AddDoorGump( m_Type ) );
					Commands.Handle( from, String.Format( "{0}Link", Commands.CommandPrefix ) );
				}
				else
				{
		...
		{
			Commands.Register( "AddDoor", AccessLevel.GameMaster, new CommandEventHandler( AddDoor_OnCommand ) );
		}
